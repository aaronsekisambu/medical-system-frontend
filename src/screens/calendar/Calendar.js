...
class Calendar extends Component {
  state = {
    ...
    selectedDay: this.props.order_by_nurse, //moment().startOf("day"),
    selectedMonthEvents: [],
    showEvents: false,
    previous: "",
    next: "",
    showCalendar: false,
    goToCurrentMonthView: false
  };
  ...
  renderWeeks = () => {
    ...
    console.log("selected =====>", currentSelectedDay)
  ...
  handleAdd = () => {
    ...
    console.log("day format", currentSelectedDate)

    let newEvents = [];

    var eventTitle = prompt("Please enter a name for your event: ");
    // const me = this.props.order_by_nurse.orders.map(order => order.schedules.map(d => d.schedules.map(t => `${d.day}: ${t}`)))
    // let event0 = {
    //   title: this.props.order_by_nurse.orders.map(order => order.service.title),
    //   date: me,
    //   dynamic: false
    // }
  ...
  render() {
    ...
    console.log("we have come here", this.props.order_by_nurse)

    if (showEvents) {
      return (
        <section className="main-calendar">
          <header className="calendar-header">
            <div className="row title-header">{this.renderDayLabel()}</div>
